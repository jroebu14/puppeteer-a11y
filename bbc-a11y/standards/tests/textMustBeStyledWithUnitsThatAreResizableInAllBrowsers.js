    ...
    function getElementFontSizes(el) {
      ...
      for (var sheetIndex in sheets) {
        ...
        for (var ruleIndex in rules) {
          var fontSize =
            rules[ruleIndex].style && rules[ruleIndex].style['font-size']
          var matchingElements = document.querySelectorAll(
            rules[ruleIndex].selectorText,
          )
          var ruleMatches =
            [].slice.apply(matchingElements).includes(el[0]) ||
            el[0].closest(rules[ruleIndex].selectorText)
