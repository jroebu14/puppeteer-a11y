module.exports = {
  ...
  failsForEach:
    'heading element (<h1> up to <h8>) that has no text content after it. ' +
  ...
  test: function ({ $, fail }) {
    ...
    function checkNextNode(heading, $nextNode) {
      var $nextIframesNodes = $nextNode.is(iframeSelector)
        ? $nextNode
        : $nextNode.find(iframeSelector)
    ...
    $(headingSelector).each(function (index, heading) {
      ...
      while (nextNode) {
        ...
        nextNode = nextNode.nextSibling
          ...
          : nextNode.parentNode && nextNode.parentNode.nextSibling
      }
