function detectTableType({ $, table }) {
  ...
  if (
    $table.parents('[contenteditable=true]').length > 0 ||
    $table.is(
      '[role=grid],[role=treegrid],[role=landmark],[summary],:has(caption,col,colgroup,tfoot,thead,th,td[header],td[scope],td[abbr],td:has(>abbr:only-child))',
    )
  ) {
    return 'data'
  }
  ...
  if (
    cols >= 5 ||
    rows.length >= 20 ||
    cells.get().some(function (cell) {
      return $(cell).css('border-width') !== '0px'
    })
  ) {
    return 'data'
  }
